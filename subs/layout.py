...
def layout_behind(
    ...
    Ключовата дума се центрира и набъбва почти до ръбовете на кадъра —
    самият растеж се орязва в рендерера, за да не излезе от видимото поле.
    Обикновените думи преди нея отиват на ред отгоре (подравнени вляво
    спрямо нея), тези след нея — на ред отдолу вдясно.
    """
    if not block.words:
        return BlockLayout(block, [], block.start, block.end)
