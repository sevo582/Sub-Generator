...
class StackStyle:
    ...
    # -- анимации на отделна дума (виж models.ANIMATIONS) ---------------
    #: „изскачане": от колко процента тръгва размерът и за колко време
    #: стига до сто.
    pop_from: float = 0.78
    pop_ms: int = 140
    #: „издигане": отдолу нагоре, отместване като дроб от височината.
    rise_by: float = 0.018
    rise_ms: int = 190
    #: „избледняване": по-бавно от обичайния преход.
    fade_in_ms: int = 320
    #: „блести": докато думата е плътна, мащабът и цветът ѝ пулсират към
    #: ``sparkle_color`` и обратно на всеки ``sparkle_ms`` — за разлика от
    #: другите три, това не е еднократен вход, а трае докато думата е на ред.
    sparkle_ms: int = 220
    sparkle_scale: float = 1.08
    sparkle_color: str = "#FFFFFF"
...
class BehindStyle:
    ...
    #: на кадъра. Заедно с плавното уголемяване думата „набъбва" почти до
    #: ръбовете, но растежът се орязва в рендерера (виж
    #: ``RasterBehindRenderer._key_growth``), така че никога не излиза
    #: извън видимия кадър. ``None`` връща фиксирания размер ``size_key``.
